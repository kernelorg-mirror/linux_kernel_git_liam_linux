		    ...
		    const struct maple_enode *parent, unsigned char slot)
{
	...
	MAS_BUG_ON(mas, p_type == maple_invalid);
	MAS_BUG_ON(mas, p_type == maple_mleaf_64);
...
void dst_setup(struct maple_copy *cp, struct ma_state *mas, enum maple_type mt)
{
	...
	if (unlikely(total == 29 && cp->max == 0x55df1a4a4fffUL))
		pr_err("dst_setup pre mas=%lx-%lx data=%d write_off=%u\n",
		       mas->index, mas->last, total, cp->write_off);
	...
	if (unlikely(total == 29 && cp->max == 0x55df1a4a4fffUL))
		pr_err("dst_setup edge split=%d left_low=%d left_high=%d shift=%d\n",
		       left_count - 1, left_low, left_high, shift);
...
void cp_data_write(struct maple_copy *cp, struct ma_state *mas)
{
	...
	s_mt = cp->src[s].mt;
	d_mt = cp->dst[d].mt;
	if (unlikely(cp->data == 29 && cp->max == 0x55df1a4a4fffUL))
		{
			void __rcu **s0 = ma_slots(cp->src[0].node, cp->src[0].mt);
			void __rcu **s1 = ma_slots(cp->src[1].node, cp->src[1].mt);
			void __rcu **s2 = ma_slots(cp->src[2].node, cp->src[2].mt);

			pr_err("cp_data_write mas=%lx-%lx split=%u write_off=%u s_count=%u d_count=%u src0=%u-%u src1=%u-%u src2=%u-%u\n",
			       mas->index, mas->last, cp->split, cp->write_off,
			       cp->s_count, cp->d_count,
			       cp->src[0].start, cp->src[0].end,
			       cp->src[1].start, cp->src[1].end,
			       cp->src[2].start, cp->src[2].end);
			pr_err("cp_data_write slots s0_end=%p s1_start=%p s1_end=%p s2_start=%p\n",
			       s0[cp->src[0].end], s1[cp->src[1].start],
			       s1[cp->src[1].end], s2[cp->src[2].start]);
		}
	do {
		dst_offset = 0;
		d_max = 0;
		...
		if (mt_is_alloc(mas->tree)) {
			...
			if (marks) {
				for (uint8_t mark = 0; mark <= MT_MARK_MAX; mark++) {
					if (ma_mark_type_set(marks, mt, mark))
						mask |= BIT(mark);
				}
			}
...
mas_wr_node_store_mark_cp(struct ma_wr_state *wr_mas, struct maple_node *node,
		unsigned char old_offset, unsigned char entry_offset,
...
static inline void mas_wr_node_store(struct ma_wr_state *wr_mas)
{
	...
	if (unlikely(mt_has_marks(mas->tree)))
		mas_wr_node_store_mark_cp(wr_mas, newnode, old_offset,
			entry_offset, offset_end, suffix_len,
			left_insert);
...
void mas_clear_mark(struct ma_state *mas, mt_mark_t mark)
{
	...
	ma_mark_clear(marks, mte_node_type(mas->node), mas->offset, mark);
}
